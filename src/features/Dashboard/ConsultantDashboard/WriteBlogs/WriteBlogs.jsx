...
const WriteBlogs = ({ userId, selectedTab }) => {
  ...
  const loadBlogs = async (page = 0, size = 10) => {
    ...
    try {
      ...
      const res = await fetchMyBlogs(page, size);
      let blogData = [];
  ...
  const loadBlogsByStatus = async (status, page = 0, size = 10) => {
    setLoadingBlogs(true);
    try {
      const res = await fetchMyBlogsByStatus(status, page, size);
      let blogData = [];
      if (res.data?.content && Array.isArray(res.data.content)) {
        blogData = res.data.content;
      } else if (Array.isArray(res.data)) {
    ...
    try {
      ...
      try {
        ...
        if (response.data && response.data.id) {
          try {
            await submitBlog(response.data.id);
            toast.success(BLOG_MESSAGES.TOAST.SUBMIT_SUCCESS);
          } catch (submitError) {
            toast.error(
